...
void ctr_fan(uint8_t s, int RGB)
{
  ...
  for (uint8_t i = 0; i < 6; i++)
  {
    ...
    case 1:
      ...
      // flow
    case 2:
      light_flow(s, RGB, flag_flow); // passes the value of flag_flow to the
...
void fan_logic(uint8_t s)
{
  uint8_t num = 0;
  for (uint8_t i = 0; i < 5; i++)
    num += (ES.fan[i].mode == ON) || (ES.fan[i].mode == Flow);
  ...
  uint8_t rand_num_use = 0, use_num = ES.timeStamp % 5 + 1; // ȷ���϶�����0
  // TODO:�Ļ���
   while (use_num--)
...
void draw_arrows(uint8_t flag, uint8_t Step, uint32_t Color)
{
  ...
  for (uint8_t w = 0; w < 7; w++)
  {
    ...
    for (uint8_t i = 1; i < 6; i++)
    {
      ...
      {
        number = (i - 2) * 2; // i number=2 i=4 number=4
